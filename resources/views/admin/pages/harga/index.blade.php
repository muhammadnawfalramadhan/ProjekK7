    ...
    <div class="overflow-x-auto border rounded-lg">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @foreach($hargaTbs as $index => $h)
                <tr class="hover:bg-green-50 transition duration-100">
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $index + 1 }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-green-700">Rp {{ number_format($h->harga_perkg, 0, ',', '.') }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ \Carbon\Carbon::parse($h->tanggal)->format('d M Y') }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                        <a href="{{ route('admin.harga.edit', $h->harga_id) }}" class="inline-flex items-center text-blue-600 hover:text-blue-800 transition duration-150 mr-3 p-2 rounded-full hover:bg-blue-50" title="Edit Data">
                        ...
                        <form action="{{ route('admin.harga.delete', $h->harga_id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data harga ini? Tindakan ini tidak dapat dibatalkan.');">
                            @csrf
                            @method('DELETE')
